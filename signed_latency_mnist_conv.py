...
if TRAIN:
    compiler = EventPropCompiler(example_timesteps=max_example_timesteps,
                                 ...
                                 reg_lambda= (1e-6,0), reg_nu_upper= 1.0, 
                                 batch_size=BATCH_SIZE, kernel_profiling=KERNEL_PROFILING)
    compiled_net = compiler.compile(network, optimisers={hidden1: {"weight": Adam(5e-2)}, output: {"weight": Adam(5e-3)}})

    with compiled_net:
        start_time = perf_counter()
        #callbacks = ["batch_progress_bar", Checkpoint(serialiser)]
        callbacks = [ SpikeRecorder(input, "sin", example_filter= [ 0, 500, 1000, 1500 ]),
                      SpikeRecorder(hidden1, "shid1",example_filter=[ 0, 500, 1000, 1500 ]),
                      #SpikeRecorder(hidden2, "shid2",example_filter=[ 0, 30000, 59999])]
                      ]
