...
LR = [2e-3, 5e-4, 2e-2, 2e-2, 1e-3 ]
ALPHABETS = None #[ 0, 2, 3, 4, 5, 6, 7 ]
...
if TRAIN:
    ...
    train_img = np.minimum((train_img-127)*2,255)
    val_img = np.minimum((val_img-127)*2,255)
    #for i in np.unique(train_labels):
        #print(i)
        #utils.show_examples(train_img, train_labels, i)
        #utils.show_examples(val_img, val_labels, i)
    #plt.show()
else:
    test_img = images
...
if TRAIN:
    ...
    compiled_net = compiler.compile(network, optimisers=optimisers, regularisers={"all_hidden_populations": SpikeCount(strength=(1e-8,1e-8),target=1)})
