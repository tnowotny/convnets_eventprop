...
from ml_genn.connectivity import Conv2D, AvgPoolConv2D, Dense
...
with network:
    ...
    for nh in range(p["HIDDEN_LAYERS"]):
        ...
        if nh > 0: print(hidden[-1].population().shape)
        if p["POOL"][nh] == 1:
            hidden.append(Layer(Conv2D(initial_hidden_weight, p["NUM_HIDDEN"][nh],
                                       p["KERNEL_SZ"][nh], flatten),
                                LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0),
                                synapse=Exponential(5.0), name=f"hidden{nh}", record_spikes= True))
        else:
            in_chn_n = 3 if nh == 0 else p["NUM_HIDDEN"][nh-1]
            initial_hidden_weight = np.random.normal(p["HID_MEAN"][nh], p["HID_SD"][nh], (p["KERNEL_SZ"][nh], p["KERNEL_SZ"][nh], in_chn_n, p["NUM_HIDDEN"][nh]))
            hidden.append(Layer(AvgPoolConv2D(initial_hidden_weight, p["NUM_HIDDEN"][nh], p["POOL"][nh],
                                              p["KERNEL_SZ"][nh], flatten), 
                                LeakyIntegrateFire(v_thresh=1.0, tau_mem=20.0),
                                synapse=Exponential(5.0), name=f"hidden{nh}", record_spikes= True))
...
with compiled_net:
    ...
    callbacks = [ SpikeRecorder(input, "sin", example_filter= [ 2, 500, 1000, 1500 ])
    ]
    val_callbacks = []
